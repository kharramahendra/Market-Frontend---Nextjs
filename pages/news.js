              ...
              return <div key={item.id} class="rounded overflow-hidden shadow-lg flex flex-col" id={item.title ? item.title.slice(0,10).replace(" ", "np") : "noid"}>
                ...
                <div class="px-6 py-3 flex flex-row items-center justify-between bg-gray-100">
                  ...
                  <span class="py-1 text-xs font-regular text-gray-900 mr-1 flex flex-row items-center">
                    <button onClick={() => { modalActivate(item.title, item.urlToImage, item.url, item.author, item.source.name, item.description, item.publishedAt, item.content) }} type="button" class="inline-flex items-center rounded-sm border border-transparent bg-gray-800 px-3 py-2 text-sm font-medium leading-4 text-white shadow-sm hover:bg-gray-900">Read More</button>
                  </span>
                </div>
              </div>
            })}
